...
class UnetDataTransform:
    ...
    def __call__(
        ...
        image = fastmri_sense.ifft2c(masked_kspace)
        ...
        # absolute value
        image = fastmri_sense.complex_abs(image)

        # apply Root-Sum-of-Squares if multicoil data
        if self.which_challenge == "multicoil":
            image = fastmri_sense.rss(image)
...
class MiniCoilTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        kspace = fastmri_sense.fft2c(
            complex_center_crop(fastmri_sense.ifft2c(to_tensor(kspace)), square_crop)
        ).numpy()
        ...
        target = fastmri_sense.rss_complex(fastmri_sense.ifft2c(to_tensor(kspace)))
