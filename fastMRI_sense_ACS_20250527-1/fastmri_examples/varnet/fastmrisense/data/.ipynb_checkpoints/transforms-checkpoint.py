...
class UnetDataTransform:
    ...
    def __call__(
        ...
        image = fastmrisense.ifft2c(masked_kspace)
        ...
        # absolute value
        image = fastmrisense.complex_abs(image)

        # apply Root-Sum-of-Squares if multicoil data
        if self.which_challenge == "multicoil":
            image = fastmrisense.rss(image)
...
class MiniCoilTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        kspace = fastmrisense.fft2c(
            complex_center_crop(fastmrisense.ifft2c(to_tensor(kspace)), square_crop)
        ).numpy()
        ...
        target = fastmrisense.rss_complex(fastmrisense.ifft2c(to_tensor(kspace)))
